...
function getCoreBall(RAPIER, world) {
  ...
  let dynamicCollider = RAPIER.ColliderDesc.ball(coreSize);
  world.createCollider(dynamicCollider, coreRigid);
  function update() {
    coreRigid.setTranslation({ x: 0, y: 0, z: 0 });
    let { x, y, z } = core.translation();
    coreMesh.position.set(x, y, z);
  }
  return { mesh: coreMesh, update };
}

export { getBody, getMouseBall, getCoreBall };